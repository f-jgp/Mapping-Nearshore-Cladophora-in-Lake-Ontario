...
The HSV-based workflow for cloud detection starts by converting an RGB image into the HSV color model, where each pixel is represented by Hue (H), Saturation (S), and Value (V). This transformation separates brightness from color information, making it more suitable for identifying visually distinctive features such as clouds.Once converted, the process focuses primarily on the S and V channels. Cloud regions are typically characterized by high brightness and low saturation. Before start both value will normalized by divide 255.Then, a pixel-wise thresholding step is applied: pixels with V above 0.75 and S below 0.2 are classified as cloud candidates and produces an  binary cloud mask. To improve robustness，removes small isolated noise and preserves larger connected regions. This step ensures that the detected cloud areas are spatially coherent. Finally, cloud coverage is computed as the ratio of cloud pixels to the total number of pixel. This provides an efficient and interpretable approximation of cloud distribution from standard RGB imagery without requiring multispectral data.
